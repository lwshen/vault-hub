...
- **Models**: `packages/model/` - Database entities (User, Vault, AuditLog, APIKey)
- **Routes**: `route/` - HTTP routing and middleware
- **Handlers**: `packages/handler/` - Request/response handling
- **Internal packages**:
...
- Database model tests (`packages/model/db_test.go`)
